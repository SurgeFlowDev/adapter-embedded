...
use surgeflow::{ArcAppState, ProjectWorkflowControl, Step, WorkflowControl};
use surgeflow::{
...
impl ProjectWorkflow for MyProjectWorkflow {
    ...
    fn entrypoint(workflow_name: WorkflowName) -> ProjectStepWithSettings<Self::Project> {
        ...
        // } else
        if workflow_name == Workflow2::NAME.into() {
...
impl Event for MyProjectEvent {
    fn value_is<T: Event + 'static>(&self) -> bool {
        match self {
            // MyProjectEvent::Workflow1(event) => event.is::<T>(),
            MyProjectEvent::Workflow2(event) => event.value_is::<T>(),
            MyProjectEvent::Immediate(immediate) => immediate.value_is::<T>(),
        }
    }
}
...
impl ProjectStep for MyProjectStep {
    ...
    //         MyProjectStep::Workflow2(step) => step.value_has_event::<T>(),
    //     }
    // }
    ...
    //         MyProjectStep::Workflow2(step) => step.value_has_event_value(event),
    //     };
    //     Ok(res)
    // }
    ...
    > {
        match self {
            ...
            MyProjectStep::Workflow2(step) => {
                let step = step
                    .run(wf.try_into()?, event.try_into()?)
                    .await
                    .expect("TODO");
                Ok(step.map(Into::into))
            }
